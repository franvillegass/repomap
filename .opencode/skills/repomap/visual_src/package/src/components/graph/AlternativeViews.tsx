...
export const ViewSwitcher = memo(function ViewSwitcher({ current, recommended, onChange }: {
  current:     ViewType
...
const HoverCard = memo(function HoverCard({ node, showFiles }: { node: Node; showFiles?: boolean }) {
...
export const OnionView = memo(function OnionView({ graph, onNodeClick }: { graph: RepoGraph; onNodeClick?: (node: Node) => void }) {
...
export const LayerStackView = memo(function LayerStackView({ graph, onNodeClick }: { graph: RepoGraph; onNodeClick?: (node: Node) => void }) {
  const [hovered,  setHovered]  = useState<string | null>(null)
  const [expanded, setExpanded] = useState<string | null>(null)

  const toggleExpanded = (id: string) => setExpanded(prev => prev === id ? null : id)

  // Build node Map for O(1) child lookups
  const nodeMap = useMemo(() => {
    const map = new Map<string, Node>()
    for (const n of graph.nodes) map.set(n.id, n)
    return map
  ...
  // Pre-compute edge counts between bands
...
export const ClusterView = memo(function ClusterView({ graph, onNodeClick }: { graph: RepoGraph; onNodeClick?: (node: Node) => void }) {
  const [hovered,  setHovered]  = useState<string | null>(null)
  const [expanded, setExpanded] = useState<string | null>(null)

  const toggleExpanded = (id: string) => setExpanded(prev => prev === id ? null : id)

  // Pre-compute clusters with Map-based edge counting
  const clusters = useMemo(() => {
    ...
    for (const e of graph.edges) {
      ...
      // Find top-level ancestor for source
      let sTop = e.source
      let sNode = nodeMap.get(sTop)
      while (sNode?.parentId && topIds.has(sNode.parentId)) {
        sTop = sNode.parentId
        sNode = nodeMap.get(sTop)
      }
      
      // Find top-level ancestor for target
      let tTop = e.target
      let tNode = nodeMap.get(tTop)
      while (tNode?.parentId && topIds.has(tNode.parentId)) {
        tTop = tNode.parentId
...
export const PipelineView = memo(function PipelineView({ graph, onNodeClick }: { graph: RepoGraph; onNodeClick?: (node: Node) => void }) {
  const [hovered,  setHovered]  = useState<string | null>(null)
  const [expanded, setExpanded] = useState<string | null>(null)

  const CARD_W = 158, CARD_H = 54, COL_GAP = 72, ROW_GAP = 8, PAD = 24

  // Build node Map for O(1) lookups — critical for performance
  const nodeMap = useMemo(() => {
    const map = new Map<string, Node>()
    for (const n of graph.nodes) map.set(n.id, n)
    return map
  ...
  // Optimized cross-edge filtering using Map lookups
  const crossEdges = useMemo(() => {
    const result = []
    let count = 0
    for (const e of graph.edges) {
      if (count >= 80) break // Hard limit for performance
