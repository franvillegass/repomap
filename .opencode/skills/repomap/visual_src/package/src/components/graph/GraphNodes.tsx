...
export const RepoNode = memo(function RepoNode(props: NodeProps<any>) {
  ...
  // Diff status (from git branch diff)
  const diffStatus = nodeData.diffStatus as string | undefined
  ...
  // Left border: diff stripe > status stripe > branch dashed > normal
  const borderLeft = diffColor
    ? `3.5px solid ${diffColor}`
    : status
  ...
  return (
    ...
    >
      ...
      {/* Label */}
      <div style={{ marginTop: 6, fontSize: 13, fontWeight: 600, color: '#e2e8f0' }}>{data.label}</div>
  ...
  return (
    <BaseEdge
      ...
      style={{
        ...
        strokeDasharray,
        opacity,
        filter: highlighted ? `drop-shadow(0 0 6px ${color}88)` : undefined,
      }}
    />
  )
})
