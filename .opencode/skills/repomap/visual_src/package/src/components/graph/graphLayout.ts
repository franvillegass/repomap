...
export interface RFNodeData extends Record<string, unknown> {
  ...
  // Git branch diff
  diffStatus?: 'added' | 'modified' | 'deleted' | 'renamed' | 'copied'
}
...
export function buildReactFlowGraph(graph: RepoGraph): {
  ...
  // Only show architectural nodes (layer + module), hide file/component noise
  ...
  const rfNodes: RFNode<RFNodeData>[] = archNodes.map((node) => ({
    id: node.id,
    type: 'repoNode',
  ...
  const rfEdges: RFEdge<RFEdgeData>[] = visibleEdges
    .filter((e) => archNodeIds.has(e.source) && archNodeIds.has(e.target))
    .map((edge) => {
...
export function buildReactFlowGraphFromResolved(
  ...
  const rfNodes: RFNode<RFNodeData>[] = archResolvedNodes.map((node) => ({
    id:   node.id,
    type: 'repoNode',
    position: positions[node.id] ?? { x: 0, y: 0 },
    data: {
  ...
  const rfEdges: RFEdge<RFEdgeData>[] = resolved.edges
    .filter((e) => archNodeIds.has(e.source) && archNodeIds.has(e.target))
    .map((edge) => ({
    id:     edge.id,
    source: edge.source,
