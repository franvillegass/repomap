...
export function BranchProvider({ baseGraph, children }: BranchProviderProps) {
  ...
  const replaceActiveBranchGraph = useCallback(async (editedGraph: RepoGraph) => {
    ...
    const nextDelta: BranchDelta = {
      ...
      removedNodeIds: parentResolved.nodes
        .map(n => n.id)
        .filter(id => !editedGraph.nodes.some(gn => gn.id === id)),
      removedEdgeIds: parentResolved.edges
        .map(e => e.id)
        .filter(id => !editedGraph.edges.some(ge => ge.id === id)),
