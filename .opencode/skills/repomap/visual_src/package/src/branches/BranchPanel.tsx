...
import type { GitBranch, RepoGraph } from '../lib/pipeline/schemas/graph'
...
interface BranchPanelProps {
  /** The full RepoGraph (needed for git branches data) */
  graph?: RepoGraph | null
  /** Called when a git branch is clicked */
  onSelectGitBranch?: (branchName: string | null) => void
  /** Currently selected git branch for diff view (null = base) */
...
export function BranchPanel({
  ...
  // Git branches from the graph data
  const gitBranches: GitBranch[] = graph?.git?.branches ?? []
    ...
    <aside style={styles.panel}>
      ...
      <div style={styles.header}>
        <span style={styles.headerLabel}>
          {gitBranches.length > 0 ? 'REPOSITORY & BRANCHES' : 'BRANCHES'}
        </span>
        <button
      ...
      {/* ── Repository (git) branches ── */}
      {gitBranches.length > 0 && (
        <>
          <div style={styles.sectionLabel}>Repository branches</div>
          {gitBranches.map((gb, i) => {
            ...
            return (
              <button
                ...
                onClick={() => onSelectGitBranch?.(isSelected ? null : gb.name)}
                title={isCurrent ? 'Current branch' : `Show diff for ${gb.name}`}
              >
                <span style={styles.gitBranchIcon}>
                  {isCurrent ? '✓' : '⑂'}
                </span>
                <span style={styles.gitBranchName}>{gb.name}</span>
                {isCurrent && <span style={styles.activePill}>current</span>}
                {isSelected && <span style={{ ...styles.activePill, background: '#3b1f1f', color: '#f87171' }}>diff</span>}
              </button>
            )
          })}
      ...
      <button
        ...
        onClick={() => {
          ...
          onSelectGitBranch?.(null)
        }}
      >
        <span style={styles.baseIcon}>◈</span>
        <span style={styles.baseName}>Base graph</span>
        {!activeGitBranch && activeBranchId === null && <span style={styles.activePill}>active</span>}
      </button>
